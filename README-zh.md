...
- **更新检查**——每天最多一次静默检查 GitHub 上的新版本（可在设置中关闭）；发现新版本时弹窗询问是否前往 GitHub Releases 页面，应用不会自行下载或安装任何内容
...
部分机型（ColorOS、MIUI、EMUI 等）的激进省电策略可能推迟或拦截定时备份。**为保证备份正常执行，建议允许 Osmium 完全后台运行，或关闭对 Osmium 的后台限制**（ColorOS：设置 → 电池 → 更多设置 → 允许 Osmium 完全后台行为）。Android 省电模式可能让备份晚几分钟执行，属正常现象。

## 安全说明

- 字段级 AES-256-GCM 加密，密钥不可导出，存于 Android Keystore
- INTERNET 权限仅用于用户自行配置的 WebDAV 备份服务器，以及每天最多一次的 GitHub 更新检查（可关闭）——无统计、无崩溃上报、无云
