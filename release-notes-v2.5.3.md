...
- **Android 17 适配**：CameraX 升级到 1.4.2（原生库已按 16 KB 页大小对齐，可在 16 KB 页大小的设备上正常安装运行）；Android 17 新增的本地网络权限会在需要时请求；应用本身仍以 targetSdk 34 运行在兼容模式，暂未提高到 37（提高会连带启用明文流量限制等行为变更，需要单独一轮真机验证）。
