...
- **安装包体积减半**：移除捆绑的闭源模型与原生库后，arm64-v8a 包由 10.6 MB 降至约 5.1 MB，其余架构同比缩小。
- **构建可复现**：发布包可以从公开源代码逐字节重建，任何人都能验证安装包确由该源代码构建（为进入纯开源软件分发渠道做准备）。
- **按架构独立版本号**：从本版起不同架构使用各自的 versionCode（601 = armeabi-v7a、602 = arm64-v8a、603 = x86_64），符合 F-Droid 对拆包应用的版本约定；功能与更新方式不变。
- 版本号 **2.5.0**，可覆盖安装 v2.4.4 及更早版本。
...
- **APK size cut roughly in half**: with the bundled closed-source model and native libraries removed, the arm64-v8a package drops from 10.6 MB to about 5.1 MB; the other ABIs shrink proportionally.
...
- **Per-ABI version codes**: each architecture now carries its own versionCode (601 = armeabi-v7a, 602 = arm64-v8a, 603 = x86_64) following the F-Droid convention for split-architecture apps; behavior and update flow are unchanged.
- Version **2.5.0** — installs over v2.4.4 and earlier versions.
