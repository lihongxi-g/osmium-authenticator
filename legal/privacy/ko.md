...
루팅 감지: 기기가 루팅된 경우, Osmium은 로컬에서 최선의 노력으로 검사(루트 관리자 패키지, 마운트, 커널 및 메모리 흔적, 부팅 상태 속성)를 실행하여 앱의 보안을 강화할지 결정합니다. 이 검사는 전적으로 이 기기에서 실행되며, 네트워크 요청을 보내지 않고, 그 결과는 저장되거나 어디로도 전송되지 않습니다. 루팅이 처음 감지되면 화면에 일회성 안내가 표시됩니다.
