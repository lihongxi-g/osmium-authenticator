...
- Prüfe empfangene Daten. Die LAN-Übertragung erfordert beide Geräte im selben WLAN: Das empfangende Gerät zeigt einen 12-stelligen Kopplungscode an, den das sendende Gerät eingeben muss, bevor etwas übertragen wird; teile den Code nur mit dem Gerät, von dem du sendest, und prüfe die Konten in der Importvorschau.
...
- Geräteintegritätsbericht. Beim Start prüft Osmium zusätzlich die Geräteintegrität lokal: System-, Mount-, Kernel-, Boot-Status- und App-Signaturprüfungen, Gegenprüfungen zwischen ihnen sowie hardwaregestützte Schlüsselattestierung (TEE-Zertifikatskette, Verified-Boot-Kette und Boot-Hash), die auf diesem Gerät mit Googles offiziellem Verifizierer verifiziert wird. Der Bericht wird nur auf deinem Gerät angezeigt; wenn das Ergebnis verdächtig oder kompromittiert ist, erinnert dich die App bei jedem Öffnen einmal daran. Eine installierte Root-Manager-App gilt als Hinweis, nicht als Beweis. Keine Softwareprüfung ist vollkommen genau; der Bericht ist daher als Orientierung zum Zustand deines Geräts zu verstehen, nicht als Garantie. Im Entwicklermodus lässt sich diese Funktion ausblenden; danach führt die App diese Prüfungen beim Start nicht mehr aus und zeigt auch keine Erinnerung mehr an.
- Entwicklermodus. Er ist hinter siebenmaligem Tippen auf den App-Namen unter „Über“ verborgen und durch eine Identitätsprüfung geschützt; er kann die Root-Einschränkungen aufheben, Einstellungseinträge ausblenden, 4/5/7-stellige Codes erlauben, deinen Tresor im Klartext exportieren und die erneute Verschlüsselung der lokalen Datenbank erzwingen. Zusätzlich bietet er ein optionales detailliertes Erkennungsprotokoll — die Integritätsprüfungen schreiben kein Protokoll, solange du es nicht aktivierst. Ein Klartext-Export enthält alle deine Geheimnisse unverschlüsselt — wer die Datei erhält, kann jedes Konto lesen. Du verwendest den Entwicklermodus auf eigenes Risiko; der Entwickler haftet nicht für Verluste, die durch den Entwicklermodus oder durch Klartext-Exporte entstehen.
...
Osmium hat kein Kontosystem und keine Cloud-Synchronisierung; alle Importfunktionen laufen vollständig auf diesem Gerät. Die App kann sich mit dem von dir konfigurierten WebDAV-Server, mit der GitHub-Releases-API für Versionsprüfungen (standardmäßig aktiviert, in den Einstellungen abschaltbar), mit der osmium.im-Website, um die neuesten Versionen dieser Nutzungsbedingungen und der Datenschutzerklärung abzurufen, mit Googles öffentlicher Statusliste für Attestierungen, wenn du die Sperrdaten auf dem Integritätsbildschirm aktualisierst, oder mit dem anderen Gerät während einer LAN-Übertragung verbinden. Die LAN-Übertragung verwendet eine aus dem Kopplungscode abgeleitete AES-256-GCM-Verschlüsselung und läuft nicht über einen Osmium-Cloudserver.
