 ...
 * (com.safekey.authenticator.integrity — K1 static probes in this release;
 * K2 consistency and K3 Key Attestation follow). This object stays as the
 * lightweight facade the UI and workers consume, so the root-hardening
 * overlay and developer-mode tools keep working unchanged.
 *
 * Scans are throttled; [refresh] with force=true is used by the
 * developer-mode "run root check". Detection results only reach the in-app
 * log when the developer-mode "detailed logging" switch is on
 * (see AppLog.detection).
 */
object RootState {
