...
class LinkTrustStore(context: Context) {
    ...
    fun isTrusted(fingerprint: String): Boolean = fingerprint.isNotBlank() && fingerprints().contains(fingerprint)

    fun setTrusted(fingerprint: String, trusted: Boolean) {
        if (fingerprint.isBlank()) return
        val next = fingerprints().toMutableSet().apply {
            if (trusted) add(fingerprint) else remove(fingerprint)
        }
