...
- 自动填充由你选择开启。若你在系统设置中选择 Osmium 作为自动填充服务，它只会在需要验证码的界面提供候选，并仅填入你点选的建议；提交前请核对目标输入框，可随时在系统设置中关闭该服务。
- 自毁 PIN 不可逆。请保持设备时间准确，并遵守你所配置的服务器与设备适用的法律和网络规则。

你可以将 Osmium 用于自己的双因素认证，以及在你控制或获授权的设备之间迁移账户。

Osmium 不运行账号体系，也不提供云同步；所有导入功能完全在本机完成。应用可能连接你配置的 WebDAV 服务器、用于可选版本检查的 GitHub Releases API，或局域网传输期间的另一台设备。局域网传输使用由配对码派生的 AES-256-GCM 加密，不经过 Osmium 云服务器。自动填充功能（如启用）同样完全在本机运行，不产生任何网络连接。
